	...
	$.post(url,{page:page,rows:rows},function(result){
		...
		for(var i=0;i<result_json.length;i++){
			$("#dg").append(
					...
					'<td>'+format(result_json[i].name_)+'</td>'+
			        '<td>'+
			        '<div class="layui-btn-group">'+
			        '<button onclick="open_edit(\''+result_json[i].id_+'\')" class="layui-btn layui-btn-mini"><i class="layui-icon">&#xe642;</i>修改</button>'+
			        '<button onclick="opend_del_dlg(\''+result_json[i].id_+'\')" class="layui-btn layui-btn-danger layui-btn-mini"><i class="layui-icon">&#xe620;</i>删除</button>'+
			        '</div>'+
			        '</td>'+
			      '</tr>'
			);
...
function add(){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['700px', '300px'],
	  content: '/group/pc/add' //iframe的url
	});
}
...
function del(ids){
	...
	$.post('/admin/group/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			refresh_data();
...
<!-- <col>  如果不设置宽 那么就是自适应 -->
  <table class="layui-table">
    <colgroup>
      <col width="50"><!-- cb -->
      <col width="180"><!-- 部门&组简称-->
      <col width="180"><!-- 部门&组名称--> 
      <col>            <!-- 操作 -->
    </colgroup>
